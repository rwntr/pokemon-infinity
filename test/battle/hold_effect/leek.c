...
{
    u32 species, genConfig, passes, trials, ability;

    PARAMETRIZE { genConfig = GEN_1; passes = 15; trials = 16; species = SPECIES_FARFETCHD; ability = ABILITY_INNER_FOCUS; }       // ~93.8% with Farfetch'd's base speed
    ...
    for (u32 j = GEN_2; j <= GEN_5; j++) {
        PARAMETRIZE { genConfig = j; passes = 1;  trials = 4;  species = SPECIES_FARFETCHD; ability = ABILITY_INNER_FOCUS; }       // 25%
    ...
    for (u32 j = GEN_6; j <= GEN_9; j++) {
        PARAMETRIZE { genConfig = j; passes = 1;  trials = 2;  species = SPECIES_FARFETCHD; ability = ABILITY_INNER_FOCUS; }       // 50%
    ...
    GIVEN {
        ...
        OPPONENT(species) { Item(ITEM_LEEK); Ability(ability); }
    } WHEN {
