...
AI_SINGLE_BATTLE_TEST("AI prefers a weaker move over a one with a downside effect if both require the same number of hits to ko")
{
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_WOBBUFFET) { HP(hp); }
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_TYPHLOSION) { Ability(ABILITY_SAND_RUSH); Moves(move1, move2, move3, move4); }
    } WHEN {
        switch (turns)
        {
        case 1:
...
{
    GIVEN {
        ASSUME(gMovesInfo[MOVE_BODY_SLAM].category == DAMAGE_CATEGORY_PHYSICAL);
...
AI_SINGLE_BATTLE_TEST("AI won't use ground type attacks against flying type Pokemon unless Gravity is in effect")
{
    GIVEN {
        ...
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_NIDOQUEEN) { Moves(MOVE_EARTH_POWER, MOVE_TACKLE, MOVE_POISON_STING, MOVE_GUST); }
    } WHEN {
        TURN { NOT_EXPECT_MOVE(opponent, MOVE_EARTH_POWER); }
        TURN { MOVE(player, MOVE_GRAVITY); NOT_EXPECT_MOVE(opponent, MOVE_EARTH_POWER); }
        TURN { EXPECT_MOVE(opponent, MOVE_EARTH_POWER); SEND_OUT(player, 1); }
    } SCENE {
