...
{//   Item                      1+  11+  21+  31+  41+  51+  61+  71+  81+  91+   Levels
    { ITEM_SUPER_POTION,    {  30,   _,   _,   _,   _,   _,   _,   _,   _,   _, } },
    { ITEM_FULL_HEAL,       {  10,  30,   _,   _,   _,   _,   _,   _,   _,   _, } },
    { ITEM_GREAT_BALL,      {  10,  10,  30,   _,   _,   _,   _,   _,   _,   _, } },
    { ITEM_REPEL,           {  10,  10,  10,  30,   _,   _,   _,   _,   _,   _, } },
    { ITEM_REVIVE,          {  10,  10,  10,  10,  30,   _,   _,   _,   _,   _, } },
    { ITEM_QUICK_BALL,      {  10,  10,  10,  10,  10,  30,   _,   _,   _,   _, } },
    { ITEM_ETHER,           {  10,  10,  10,  10,  10,  10,  30,   _,   _,   _, } },
    { ITEM_HYPER_POTION,    {   5,  10,  10,  10,  10,  10,  10,  30,   _,   _, } },
    { ITEM_CHERISH_BALL,    {   4,   4,  10,  10,  10,  10,  10,  10,  30,  10, } },
    { ITEM_MAX_ETHER,       {   _,   4,   4,  10,  10,  10,  10,  10,  10,   _, } },
    ...
    { ITEM_BOTTLE_CAP,      {   _,   _,   _,   4,   4,  10,  10,  10,  10,  10, } },
    { ITEM_PEARL_STRING,    {   _,   _,   _,   1,   1,   _,   _,   _,   _,   _, } },
    { ITEM_MAX_REVIVE,      {   _,   _,   _,   _,   4,   4,  10,  10,  10,  10, } },
    { ITEM_BALM_MUSHROOM,   {   _,   _,   _,   _,   1,   1,   _,   _,   _,   _, } },
    { ITEM_LEFTOVERS,       {   _,   _,   _,   _,   _,   4,   4,  10,  10,  10, } },
    { ITEM_PP_MAX,          {   _,   _,   _,   _,   _,   1,   1,   _,   4,  10, } },
    { ITEM_ELIXIR,          {   _,   _,   _,   _,   _,   _,   1,   1,   _,  10, } },
    { ITEM_PP_UP,           {   _,   _,   _,   _,   _,   _,   _,   4,   4,   _, } },
    { ITEM_BIG_NUGGET,      {   _,   _,   _,   _,   _,   _,   _,   1,   1,  10, } },
    { ITEM_GOLD_BOTTLE_CAP, {   _,   _,   _,   _,   _,   _,   _,   _,   1,   4, } },
    { ITEM_MAX_ELIXIR,      {   _,   _,   _,   _,   _,   _,   _,   _,   _,   4, } },
    { ITEM_COMET_SHARD,     {   _,   _,   _,   _,   _,   _,   _,   _,   _,   1, } },
    { ITEM_SACRED_ASH,      {   _,   _,   _,   _,   _,   _,   _,   _,   _,   1, } },
};
...
static bool32 CheckIfCanFireTwoTurnMoveNow(u8 battler, bool8 checkChargeTurnEffects)
{
    ...
    if ((GetMoveEffect(gCurrentMove) == EFFECT_SOLAR_BEAM) && (GetBattlerAbility(gBattlerAttacker) == ABILITY_CHLOROPLAST))
        return TRUE;
...
void ApplyExperienceMultipliers(s32 *expAmount, u8 expGetterMonId, u8 faintedBattler)
{
    ...
    s32 highestPartyMonLv;

    if (IsTradedMon(&gPlayerParty[expGetterMonId]))
        *expAmount = (*expAmount * 150) / 100;
    if (holdEffect == HOLD_EFFECT_TRAINING_BAND) {
        highestPartyMonLv = GetHighestLevelInPlayerParty();
        if (GetMonData(&gPlayerParty[expGetterMonId], MON_DATA_LEVEL) < (highestPartyMonLv - 4))
            *expAmount *= 5;
    }
